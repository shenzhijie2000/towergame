...
//	}
  ...
  animate(time) {
    ...
    //console.log(`engine: animate: ${time}`)
    if (this.paused) {
      setTimeout(() => {
  ...
  getTimeMovement(name, value, render, option = {}) {
    ...
    if (!movement.processing) {
      ...
      value.forEach((v) => {
        ...
        console.log(`engine: name = ${name}, movementInstanceName = ${movementInstanceName}, start = ${parseFloat(v[0])}, end = ${parseFloat(v[1])}`)
      })
      before && before()
    }
      ...
      if (this.assetsErrorQueue.length > 0) {
        this.assetsErrorQueue.forEach((i) => {
          ...
          if (retry >= this.loadLimit) {
            ...
            console.log(`engine: retry`)
          } else if (type === 'image') {
            this.addImg(name, src, retry)
            console.log(`engine: addImg`)
          } else {
            this.addAudio(name, src, retry)
            console.log(`engine: addAudio`)
          }
        })
        this.assetsErrorQueue = []
      }
